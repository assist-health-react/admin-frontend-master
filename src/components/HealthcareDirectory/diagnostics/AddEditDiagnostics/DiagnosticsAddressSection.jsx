...
export default function DiagnosticsAddressSection({ formData, setFormData }) {
  ...
  const removeAddress = (i) => {
    setFormData((prev) => ({
      ...prev,
      addresses: prev.addresses.filter((_, index) => index !== i),
    }));
  };
  ...
  return (
    <div className="bg-white border p-5 rounded-xl shadow-sm">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">Service Coverage (Address) *</h3>
      ...
      {formData.addresses.map((addr, index) => (
        <div key={index} className="border rounded-xl p-4 mb-4 bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
            <div>
              <label className="text-sm mb-1 block">Location Name *</label>
              <input
                value={addr.locationName}
                onChange={(e) =>
            ...
            <div>
              <label className="text-sm mb-1 block">Area *</label>
              <input
                value={addr.area}
                onChange={(e) => updateAddress(index, "area", e.target.value)}
                className="border rounded-lg w-full px-3 py-2"
