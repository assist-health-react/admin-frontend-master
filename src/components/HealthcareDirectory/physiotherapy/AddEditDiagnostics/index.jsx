...
import PhysiotherapyServicesSection from "./PhysiotherapyServicesSection";
import IntroductionSection from "./IntroductionSection";
import PhysiotherapyAddressSection from "./PhysiotherapyAddressSection";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && initialData?._id) {
        response = await healthcareService.updatePhysiotherapy(
          initialData._id,
          payload
        );
      } else {
        response = await healthcareService.createPhysiotherapy(payload);
      }

      if (response?.status === "success") {
        showSnackbar(
          isEditing
            ? "Physiotherapy Updated Successfully!"
            : "Physiotherapy Added Successfully!",
          "success"
        );
        onSuccess?.();
        onClose?.();
      } else {
        throw new Error(response?.message || "Failed to save physiotherapy");
      }
    } catch (err) {
      console.error(err);
      showSnackbar(err.message || "Failed to save physiotherapy", "error");
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Services */}
          <PhysiotherapyServicesSection
            formData={formData}
            setFormData={setFormData}
          />

          {/* Coverage Addresses */}
          <PhysiotherapyAddressSection
            formData={formData}
            setFormData={setFormData}
          />
