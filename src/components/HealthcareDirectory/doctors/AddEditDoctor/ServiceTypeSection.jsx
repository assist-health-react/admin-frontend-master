...
const ServiceTypeSection = ({
  ...
  // ---------------------------
  // PREFILL PINCODE IN EDIT MODE
  // ---------------------------
  useEffect(() => {
    if (formData.areas && formData.areas.length > 0) {
      setServicePincode(formData.areas[0].pincode || "");
    }
  }, []);

  // ---------------------------
  // UPDATE REGIONS WHEN AREAS CHANGE
  // ---------------------------
  useEffect(() => {
    const input = document.querySelector('input[name="servicePincode"]');

    const currentPincode = input ? input.value : "";

    const safeAreas = (formData.areas || []).filter(
      a => a && a.pincode && a.region
    );

    const regions = safeAreas
      .filter(area => area.pincode === currentPincode)
      .map(area => ({
        value: area.region,
        label: area.region
  ...
  const handleServicePincodeInputChange = e => {
    ...
    // Clear selections if user changes pincode
    if (value !== servicePincode) {
      setCurrentRegions([]);
    }
  };
        ...
        <div className="mb-4">
          ...
          {formData.areas.length > 0 && (
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {(formData.areas || []).map((area, index) => {
                  if (!area || !area.pincode || !area.region) return null;
