...
import HomecareDetailsModal from "./HomecareDetailsModal";
...
const [selectedHomecare, setSelectedHomecare] = useState(null);
  ...
  <div className="p-5 flex flex-col h-full">
    ...
    <div className="flex gap-4 mb-3">
      ...
      <div className="min-w-0">
        <h3 className="text-base font-semibold truncate">{hc.name}</h3>
        <p className="text-sm text-gray-500">{hc.phone}</p>
        <p className="text-sm text-gray-500">
          {hc.address?.city}, {hc.address?.area}
        </p>
      </div>
    </div>
    ...
    <div className="text-sm text-gray-600 mb-4 space-y-1">
      <p>
        <strong>Type:</strong> {hc.servicetype || "—"}
      </p>
      <p className="truncate">
        <strong>Services:</strong>{" "}
        {(hc.services || []).slice(0, 2).join(", ")}
        {hc.services?.length > 2 && " +more"}
      </p>
    </div>
    ...
    <div className="mt-auto flex gap-2 pt-3 border-t">
      ...
      <button
        onClick={() => setSelectedHomecare(hc)}
      ...
      <HomecareDetailsModal
        isOpen={!!selectedHomecare}
        homecare={selectedHomecare}
