...
import IntroductionSection from "./IntroductionSection";
import HomecareAddressSection from "./HomecareAddressSection";
import { languageOptions } from "../utils";
...
const AddEditDiagnostics = ({ onClose, initialData, isEditing, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    website: initialData?.website || "",
    addressStreet:
      initialData?.address?.street || initialData?.address || "",
    area: initialData?.address?.area || initialData?.area || "",
    ...
    introduction: initialData?.introduction || "",
    addresses:
      initialData?.addresses && initialData.addresses.length
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && initialData?._id) {
        response = await healthcareService.updateHomecare(
          initialData._id,
          payload
        );
      } else {
        response = await healthcareService.createHomecare(payload);
      }

      if (response?.status === "success") {
        showSnackbar(
          isEditing
            ? "Homecare Updated Successfully!"
            : "Homecare Added Successfully!",
          "success"
        );
        onSuccess?.();
        onClose?.();
      } else {
        throw new Error(response?.message || "Failed to save homecare");
      }
    } catch (err) {
      console.error(err);
      showSnackbar(err.message || "Failed to save homecare", "error");
    } finally {
      setIsSubmitting(false);
    }
  };
